...
public class Faction implements INBTSerializable<NBTTagCompound> {
    ...
    private String  playerFinderMessage = "No player found.";
    private int  playerFinderCooldown;
    ...
    public String getFinderMesage() {
        return playerFinderMessage;
    }
    ...
    public void tickPlayerFinder() {
        playerFinderCooldown--;
        if (playerFinderCooldown == 0) {
            EventHandlerFaction.broadcastToFaction(this, "Player finder refreshed", 0);
        }
    }

    public String findPlayer(Random rand)
    {
        if(playerFinderCooldown <= 0) {
            playerFinderCooldown = ConfigGeneral.getInt("player_finder_refresh_timer");
            playerFinderCooldown = ConfigGeneral.getInt("player_finder_refresh_timer");
            EntityPlayerMP[] availablePlayers = ServerUtils.getPlayers().stream().filter(player ->
            {
                UUID playerUUID = player.getUniqueID();
                if (!EventHandlerFaction.hasUserFaction(playerUUID))
                    return true;
                List<DimensionalPosition> poses = EventHandlerFaction.getFaction(EventHandlerFaction.getFactionOf(playerUUID)).getChunks();
                if (isMember(playerUUID)) {
                    return false;
                }
                for (DimensionalPosition pos : poses) {
                    if (player.dimension == pos.getDimension() && player.chunkCoordX == pos.getPos().x && player.chunkCoordZ == pos.getPos().z) {
                        return false;
                    }
                }
                return true;
            ...
            if (availablePlayers.length == 0) {
                playerFinderMessage = "No player found.";
            } else {
                EntityPlayerMP player = availablePlayers[rand.nextInt(availablePlayers.length)];
                int noise = ConfigGeneral.getInt("player_finder_noise");
                int posX = MathHelper.floor(player.posX) + noise / 2 - rand.nextInt(noise);
                int posZ = MathHelper.floor(player.posZ) + noise / 2 - rand.nextInt(noise);
                playerFinderMessage = "Found player near: " + posX + ", " + posZ;
            }
        }
        return playerFinderMessage;
    }
