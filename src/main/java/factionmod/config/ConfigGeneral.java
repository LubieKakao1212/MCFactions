...
public class ConfigGeneral {
    ...
    public static void loadFromConfig(final Configuration config) {
        ...
        p = config.get(CAT, "player_finder_noise", 32);
        p.setComment("How much randomness to add to player finder positions");
        p.setMinValue(0);
        p.setMaxValue(1024);
        INT_VALUES.put("player_finder_noise", p.getInt());

        p = config.get(CAT, "player_finder_refresh_timer", 6000);
        p.setComment("How frequently does player finder refresh, in ticks");
        p.setMinValue(20);
        p.setMaxValue(Integer.MAX_VALUE);
        INT_VALUES.put("player_finder_refresh_timer", p.getInt());
